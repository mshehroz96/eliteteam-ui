...
document.addEventListener("DOMContentLoaded", function () {
  (function () {
    ...
    function fetchEvents(info, successCallback) {
      ...
      var strSelectedJobTitleFilter = " " + $("#ddlJobList").val();
      var arrSelectedEvents = [];
      for (var i = 0; i < selectedEvents.length; i++) {
        var strJobTitle = selectedEvents[i].extendedProps.job_title;
        if (strJobTitle == "[MyAvailability]" || strJobTitle == "[ClientMeeting]" || strSelectedJobTitleFilter.indexOf(strJobTitle) > 0)
        {
          arrSelectedEvents.push(selectedEvents[i]);
        }
      }
      if (arrSelectedEvents.length == 0 ) { arrSelectedEvents = selectedEvents;  }
